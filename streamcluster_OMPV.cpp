...
float dist2(float *coord, float *coord1, int dim);
...
#pragma omp parallel for simd reduction(+ : result)
  for (int i = 0; i < dim; i++)
    result += (coord[i] - coord1[i]) * (coord[i] - coord1[i]);
  return (result);
}
...
double pgain(long x, Points *points, double z, long int *numcenters) {
  ...
  float xcost[points->num];

#pragma omp parallel for simd
  for (int i = 0; i < points->num; i++)
    xcost[i] = dist2(points->p[i].coord, points->p[x].coord, points->dim) *
               points->p[i].weight;
  ...
  for (i = 0; i < points->num; i++) {
    ...
    if (xcost[i] < current_cost) {
      ...
      cost_of_opening_x += xcost[i] - current_cost;
      ...
      lower[center_table[assign]] += current_cost - xcost[i];
    }
  }
...
void streamCluster(PStream *stream, long kmin, long kmax, int dim,
  ...
  while (1) {
    ...
    ElapsedTime += (double)(End.tv_sec * 1000000 + End.tv_usec -
...
int main(int argc, char **argv) {
  ...
  if (argc != 9) {
    fprintf(stderr,
            "usage: %s k1 k2 d n chunksize clustersize infile outfile\n",
            argv[0]);
